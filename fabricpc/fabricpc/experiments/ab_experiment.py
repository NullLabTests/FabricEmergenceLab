...
DataLoaderFactory = Callable[[int], Tuple[Any, Any]]  # seed -> (train_loader, test_loader)
...
class ABResults:
    ...
    def print_summary(self) -> None:
        ...
        is_rate = np.all(a_vals >= 0) and np.all(a_vals <= 1) and np.all(b_vals >= 0) and np.all(b_vals <= 1)
        ...
        if b_t.mean > 0:
            print(f"Ratio: {self.arm_a_name} is {a_t.mean / b_t.mean:.2f}x {self.arm_b_name} time")
        ...
        for i in range(self.n_trials):
            ...
            print(f"{i + 1:<8} {self.seeds[i]:<8} {a_vals[i] * scale:<20.2f} {b_vals[i] * scale:<20.2f} {diff:<+12.2f}")
        print("-" * len(header))
        ...
        print()
        print(
            f"{self.arm_a_name}: {a_stats.mean:.2f} +/- {a_stats.se:.2f}{pct}  (mean +/- SE, SD={a_stats.std:.2f}{pct})"
        )
        print(
            f"{self.arm_b_name}: {b_stats.mean:.2f} +/- {b_stats.se:.2f}{pct}  (mean +/- SE, SD={b_stats.std:.2f}{pct})"
        )
        ...
        if self.n_trials >= 2:
            ...
            print()
            print("--- Paired t-test ---")
            print(f"Mean difference ({self.arm_a_name} - {self.arm_b_name}): {ttest.mean_difference * scale:+.2f}{pct}")
            print(f"t-statistic: {ttest.t_statistic:.4f}")
            print(f"p-value: {ttest.p_value:.4f}, N = {ttest.n}")
            print(f"Significant at p<0.05: {'YES' if ttest.significant_at_05 else 'NO'}")
            ...
            if req_n >= 999999:
                ...
                print(f"  -> Current n_trials ({self.n_trials}) may be underpowered. Consider increasing to {req_n}.")
        else:
            print()
            print("--- Statistical tests require n_trials >= 2 ---")
            diff = float(np.mean(a_vals - b_vals))
            print(f"Mean difference ({self.arm_a_name} - {self.arm_b_name}): {diff * scale:+.2f}{pct}")
...
class ABExperiment:
    ...
    def _run_arm_trial(
        ...
        if self.metric not in metrics:
            ...
            raise KeyError(
                f"Metric '{self.metric}' not found in eval results for arm '{arm.name}'. Available: {available}"
            )
    ...
    def run(self) -> ABResults:
        ...
        for trial_idx in range(self.n_trials):
            ...
            result_a = self._run_arm_trial(self.arm_a, trial_seed, train_loader, test_loader)
            arm_a_trials.append(result_a)
            print(
                f"  {self.arm_a.name}: {self.metric}={result_a.metric_value:.4f}  (train: {result_a.train_time:.1f}s)"
            )

            # Arm B (recreate loaders for isolation)
            train_loader, test_loader = self.data_loader_factory(trial_seed)
            result_b = self._run_arm_trial(self.arm_b, trial_seed, train_loader, test_loader)
            arm_b_trials.append(result_b)
            print(
                f"  {self.arm_b.name}: {self.metric}={result_b.metric_value:.4f}  (train: {result_b.train_time:.1f}s)"
            )
